...
window.renderBarplot = async function (data) {
  ...
  const dataDetails = data[0].detail ? data.map(d => d.detail[0].value).filter((value, index, array) => array.indexOf(value) === index) : data;
  ...
  groupedDataByDetails = data[0].detail ? groupedDataByDetails : [groupedDataByDetails];


  const dataColors = data[0].color ? data.map(d => d.color[0].value).filter((value, index, array) => array.indexOf(value) === index) : data;
  let groupedDataByColors = [];
  dataColors.forEach((color) => {
    groupedDataByColors.push(data[0].color
        ? data.filter((value, index, array) => value.color[0].value === color)
        : color
    );
  });
  groupedDataByColors = data[0].color ? groupedDataByColors : [groupedDataByColors];
  ...
  //   .style("alignment-baseline", "middle")
  //   .style("text-anchor", (dataRow) => {
  //     const cos = Math.cos(2 * Math.PI / data.length * data.indexOf(dataRow) - Math.PI / 2);
  //     const sin = Math.sin(2 * Math.PI / data.length * data.indexOf(dataRow) - Math.PI / 2);
  //     if (cos > 0.00001) {
  //       return "start";
  //     } else if (cos < -0.00001) {
  //       return "end";
  ...
  //   })
  //   .attr("x", (dataRow) => {
  //     const radius = (innerRadius + (outerRadius - innerRadius) * dataRow.value[0].value) + 15;
  ...
  //   })
  //   .text(dataRow => dataRow.value[0].formattedValue);
